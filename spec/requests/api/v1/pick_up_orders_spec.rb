...
describe 'pick up orders' do
  ...
  describe 'GET index' do
    ...
    context 'when the request is filtering by commodity id' do
      let(:filters) do
        {
          contract_commodity_id_eq: pick_up_orders.sample.contract.CommodityId
        }
      end
      ...
      it 'should only return matching records' do
        ...
        expect(parsed_body.map { |hash| hash['contract']['commodity_id'] })
          .to all eq filters[:contract_commodity_id_eq].to_i
      end
    end
    ...
    context 'when the request is filtering by contract id' do
      let(:filters) do
        {
          contract_contract_id_eq: pick_up_orders.sample.ContractId
        }
      end
      ...
      it 'should only return matching records' do
        ...
        expect(parsed_body.map { |hash| hash['contract']['contract_id'] })
          .to all eq filters[:contract_contract_id_eq]
      end
    end
